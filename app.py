...
import os
import io
from ftplib import FTP  # ➔ サーバー（FFFTPの宛先）に別ファイルとして自動保存するための部品
...
# 【1ヶ月・1日・公開保存用】HTMLテーブル生成関数
def make_html_table_with_time(day_schedule, font_size="11px", padding="3px"):
    html = []
    html.append(f"<table class='calendar-table' style='border-collapse: collapse; text-align: center; font-size: {font_size}; font-family: sans-serif; width: 100%; table-layout: fixed;'>")
    html.append("<tr style='background-color: #f0f0f0; font-weight: bold; border-bottom: 2px solid #ccc;'>")
    ...
    for hour in hours_sequence:
        r1 = day_schedule[hour]["row1"]
        r2 = day_schedule[hour]["row2"]
        
        # XX:00
        html.append("<tr style='border-bottom: 1px dashed #ddd;'>")
        html.append(f"<td style='border: 1px solid #ccc; background-color: #f9f9f9; font-weight: bold; padding: {padding} 0;'>{hour}:00</td>")
        html.append(f"<td style='border: 1px solid #ccc; background-color: #ffffff;'></td><td style='border: 1px solid #ccc; font-weight: bold; background-color: {get_bg(r1['h1'], 'h1')}'>{wrap_name(r1['h1'], 'h1')}</td>")
        html.append(f"<td style='border: 1px solid #ccc; background-color: #ffffff;'></td><td style='border: 1px solid #ccc; font-weight: bold; background-color: {get_bg(r1['h2'], 'h2')}'>{wrap_name(r1['h2'], 'h2')}</td>")
        html.append(f"<td style='border: 1px solid #ccc; background-color: #ffffff;'></td><td style='border: 1px solid #ccc; font-weight: bold; background-color: {get_bg(r1['h3'], 'h3')}'>{wrap_name(r1['h3'], 'h3')}</td>")
        html.append("</tr>")
        
        # XX:30
        html.append("<tr style='border-bottom: 1px solid #ccc;'>")
        time_label = "0:30" if hour == 0 else f"{hour}:30"
        html.append(f"<td style='border: 1px solid #ccc; background-color: #f9f9f9; color: #aaa; padding: {padding} 0;'>{time_label}</td>")
        html.append(f"<td style='border: 1px solid #ccc; background-color: #ffffff;'></td><td style='border: 1px solid #ccc; font-weight: bold; background-color: {get_bg(r2['h1'], 'h1')}'>{wrap_name(r2['h1'], 'h1')}</td>")
        html.append(f"<td style='border: 1px solid #ccc; background-color: #ffffff;'></td><td style='border: 1px solid #ccc; font-weight: bold; background-color: {get_bg(r2['h2'], 'h2')}'>{wrap_name(r2['h2'], 'h2')}</td>")
        html.append(f"<td style='border: 1px solid #ccc; background-color: #ffffff;'></td><td style='border: 1px solid #ccc; font-weight: bold; background-color: {get_bg(r2['h3'], 'h3')}'>{wrap_name(r2['h3'], 'h3')}</td>")
        html.append("</tr>")
    html.append("</table>")
    return "".join(html)
    ...
    with c1:
        if st.button("🚀 この内容をWeb稼働表として公開保存する（確定送信）", use_container_width=True):
            with st.spinner("サーバーのファイルを更新中..."):
                ...
                if success:
                    st.success(f"🟢 保存完了！『/Flower_House/sync_shift_System/』へダイレクトに反映されました。")
                else:
                    st.error(f"🔴 保存に失敗しました。FTP設定内容を再度確認してください: {err_msg}")
    with c2:
        # 手元のPC・スマホからその場で即座に印刷・PDF化を呼び出すボタン
        st.markdown('<button onclick="window.print()" style="width:100%; height:40px; background-color:#4CAF50; color:white; border:none; border-radius:4px; cursor:pointer; font-size:14px; font-weight:bold;">📄 この画面をPDF保存</button>', unsafe_allow_html=True)


# ─── 表示切り替えタブ ───
view_mode = st.tabs(["📊 1ヶ月表示（カレンダー）", "📅 1週間表示（時間軸スリム化）", "🔍 1日集中表示"])
        ...
        for day_of_week in range(7):
            ...
            with row_cols[day_of_week]:
                if start_offset <= current_cell_idx and day_pointer <= max_days:
                    ...
                    table_html = make_html_table_with_time(calendar_data[day_pointer], font_size="11px", padding="3px")
                    st.html(f"<div style='border: 1px solid #999; height: 430px; overflow-y: auto; background-color: #ffffff; border-radius: 4px;'>{table_html}</div>")
                    day_pointer += 1
                else:
...
with view_mode[1]:
    ...
    html_sheet = []
    html_sheet.append("<table class='calendar-table'>")
    ...
    for hour in hours_sequence:
        # XX:00の行
        html_sheet.append("<tr style='border-bottom: 1px dashed #ddd;'>")
        html_sheet.append(f"<td style='border: 1px solid #ccc; background-color: #f2f2f2; font-weight: bold; padding: 5px 0;'>{hour}:00</td>")
        
        for day_of_week in range(7):
            current_d = start_d + day_of_week
            if 1 <= current_d <= max_days:
                r1 = calendar_data[current_d][hour]["row1"]
                html_sheet.append(f"<td style='border: 1px solid #eee; background-color: #ffffff;'></td><td style='border: 1px solid #ccc; font-weight: bold; background-color: {get_bg(r1['h1'], 'h1')}'>{wrap_name(r1['h1'], 'h1')}</td>")
                html_sheet.append(f"<td style='border: 1px solid #eee; background-color: #ffffff;'></td><td style='border: 1px solid #ccc; font-weight: bold; background-color: {get_bg(r1['h2'], 'h2')}'>{wrap_name(r1['h2'], 'h2')}</td>")
                html_sheet.append(f"<td style='border: 1px solid #eee; background-color: #ffffff;'></td><td style='border: 1px solid #ccc; font-weight: bold; background-color: {get_bg(r1['h3'], 'h3')}'>{wrap_name(r1['h3'], 'h3')}</td>")
            else:
                html_sheet.append("<td colspan='6' style='border: 1px solid #eee; background-color: #fdfdfd; opacity:0.2;'></td>")
        html_sheet.append("</tr>")
        
        # XX:30の行
        html_sheet.append("<tr style='border-bottom: 1px solid #ccc;'>")
        time_label = "0:30" if hour == 0 else f"{hour}:30"
        html_sheet.append(f"<td style='border: 1px solid #ccc; background-color: #f2f2f2; color: #777;'>{time_label}</td>")
...
with view_mode[2]:
    ...
    with d_col3:
        if st.button("次の日 →", use_container_width=True, key="next_day_btn"):
            if st.session_state.current_day_val < max_days:
                st.session_state.current_day_val += 1
    with d_col2:
        select_day = st.slider("日選択", min_value=1, max_value=max_days, value=st.session_state.current_day_val, key="day_slider_bar")
    ...
    large_table_html = make_html_table_with_time(calendar_data[st.session_state.current_day_val], font_size="14px", padding="6px")
    st.html(f"""
        <div style='max-width: 600px; margin: 0 auto; border: 2px solid #1c83e1; background-color: #ffffff; border-radius: 8px; padding: 10px;'>
            {large_table_html}
        </div>
    """)